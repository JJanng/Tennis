...
st.markdown("""
    ...
    /* 입력창 여백 보정 */
    div[data-testid="stNumberInput"] { margin-top: -5px !important; margin-bottom: -5px !important; }
    hr { margin: 5px 0px !important; }
    </style>
    """, unsafe_allow_html=True)

st.markdown("<h1 style='font-size: 19px; text-align: left;'>🎾 서울산 테니스클럽 대회 운영 시스템</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.divider()
    num_groups = st.number_input("월례대회 총 그룹 수", 1, 10, 3)
    st.sidebar.info("""
    **1. 한울 AA (개인)**
    * 매 경기 파트너와 상대가 바뀜
    * 개인별 승률 및 득실 계산 결정
